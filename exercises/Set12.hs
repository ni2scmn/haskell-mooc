...
count v f = foldr (\x acc -> if x==v then acc + 1 else acc) 0 f
...
inBoth a b = foldr (\v acc -> if v `elem` b then v:acc else acc) [] a
...
instance Foldable List where
  ...
  -- foldr f acc l@(LNode a x) = foldr f (f a acc) x
