...
treeMax Empty = 0
treeMax (Node x l r) = max (max (treeMax l) (treeMax r)) x
...
set _ _ Empty = Empty
set (t:ts) val (Node x l r) = Node x (set ts val l) (set ts val r) where 
   set_left = case t of
                StepL -> Node x (set ts val l) r
                StepR -> Node x r (set ts val r)
